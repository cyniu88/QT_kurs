...
std::optional<int> iDom_Client::getMinutesDialog(QString name)
{
    std::optional<int> r;
    bool ok;
    int number = QInputDialog::getInt(this, name, "Wprowadź liczbę:", 0, 0, 100, 1, &ok);
    if(ok ==true)
        r = number;
...
void iDom_Client::on_b_lightDomek_released()
{
    QString bulbID = "98";
    QString command;
    QString lightState = ui->b_lightDomek->isChecked() ? "on" : "off";

    if (pressTime.elapsed() < 220) {
        command = QString("light bulb %1 %2").arg(lightState, bulbID);
    }
    else {

        auto number = getMinutesDialog("podaj ilość minut");
        if (number.has_value()) {
            command = QString("gateway sleep %2 light bulb %1 %3").arg(lightState, QString::number(number.value()), bulbID);
        }
    }
    if(command.isEmpty() == false)
        emit sendTCP("console", command.toStdString());

    droid.vibrate(200);
}


void iDom_Client::on_b_lightDomek_pressed()
{
    pressTime.start();
}


void iDom_Client::on_b_lightPokojN_pressed()
{
    pressTime.start();
}


void iDom_Client::on_b_lightPokojN_released()
{
    QString bulbID = "128";
    QString command;
    QString lightState = ui->b_lightPokojN->isChecked() ? "on" : "off";
